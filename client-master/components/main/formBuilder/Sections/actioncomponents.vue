          ...
          <v-container>
            <v-row>
              <draggable v-if="formBuilderFields.length>0" class="dragArea list-group" group="fields"
                ...
                <template v-for="element in formBuilderFields">
                  <template v-if="element.TFF_FDelete==0">
                    
                   <Actioncomponent :element="element" :selected="selected==element" @deleteField="deleteField" :key="element.TFF_FID"
                      @copyField="copyField" @select2="select2(element)" @setting="$emit('setting', element)">
                    </Actioncomponent>
...
import Actioncomponent from'./componentsSections/actionComponent.vue';
export default {
  components: { draggable,Actioncomponent },

  props: ["formBuilderFields", "readonly"],
  data() {
    return {
