<template>
    <v-card tile v-if="data.TPS_FIDs_NumberList && data.TPS_FIDs_NumberList.length > 0">
        <v-list nav dense>
            <v-list-item-group v-model="selectedItem" color="primary" :readonly="readonly">
                <v-list-item v-for="(item, i) in data.TPS_FIDs_NumberList" :key="i">
                    ...
                    <v-list-item-icon v-if="selectedItem == i" @click="deleteItem(i)">
                        <v-icon v-if="!readonly" color="pink">mdi-delete-forever</v-icon>
                    </v-list-item-icon>
...
export default {
    ...
    methods: {
        ...
        setAsDefault(item) {
            ...
            if (!this.readonly) {
                this.data.TPS_FNumberDefault = item
            }
        }
    }
}
