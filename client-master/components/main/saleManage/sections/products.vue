<template>
  <v-expansion-panel class="togglebox">
    ...
    <v-expansion-panel-content>
      ...
      <v-row class="mb-4">
        <SalePageAddProduct v-if="showAddDialog" :salePage="data" :formData="formData" :defaults="formDefaults"
        ...
        <SalePageAlertDialog v-if="showDuplicateDialog" title="ایجاد کپی از محصول"
          comment="شما قصد کپی از محصول را دارید.آیا تایید میکنید؟" @submit="duplicateProduct"
          @cancel="showDuplicateDialog = false" />

        <v-col cols="2">
          <v-btn v-if="!readonly" :loading="insertProductLoading" dark color="teal" depressed @click="insertProduct">تعریف
            محصول جدید</v-btn>
        </v-col>

        <v-col cols="10" v-if="data.products && data.products.length == 0">
      ...
      <v-row>
        <v-col cols="12">
          <ProductsTable v-if="data.products && data.products.length > 0" :salePage="data" @editProduct="editProduct"
            @duplicateProduct="duplicateDialog" :formDefaults="formDefaults" :readonly="readonly"
...
import SalePageAddProduct from "../dialog/SalePageAddProduct.vue";
import ProductsTable from "./productSections/productsTable.vue";
...
export default {
  ...
  methods: {
    ...
    async editProduct(item) {
      ...
      const localProduct = this.data.products.find(p => p.TGO_FID == item.TGO_FID)
      if (localProduct) {
    ...
    async duplicateProduct() {
      try {
        ...
        const productOptionValue = this.data.productsOptionValue.filter(pov => pov.TGPV_FID_Product == this.itemForDuplicate.TGO_FID && pov.TGPV_FDelete == 0)
        if (productOptionValue && productOptionValue.length > 0) {
          const newProductOptionValue = JSON.parse(JSON.stringify(productOptionValue))
  ...
  components: { SalePageAddProduct, ProductsTable, SalePageAlertDialog }
};
</script>
