<template>
  <v-data-table :items="salePage.products" :headers="readonly ? productHeadersReadonly : productHeaders"
    ...
    <template v-slot:item.TGO_FOrder="{ item }">
      <UpDownButtons v-if="item.TGO_FDelete == 0" :items="salePage.products" :item="item" FieldName="TGO_FOrder"
        :readonly="readonly">
      </UpDownButtons>
    </template>

    <template v-slot:item.TGO_FActive="{ item }">
      <v-switch v-if="item.TGO_FDelete == 0 && !readonly" v-model="item.TGO_FActive" :true-value="1" :false-value="0"
        inset></v-switch>
      <v-icon v-else-if="item.TGO_FActive" color="green" size="x-large">mdi-check</v-icon>
    </template>
    ...
    <template v-slot:item.formButtons="{ item }">
      <v-row>
        <v-col cols="4">
          <v-btn v-if="!readonly" @click="$emit('duplicateProduct', item)" icon color="amber accent-4">
        ...
        <v-col cols="4">
          <DeleteButton v-if="!readonly" :items="salePage.products" :item="item" state="product"></DeleteButton>
        </v-col>
      </v-row>
    </template>
    ...
    <template v-slot:expanded-item="{ item }">
      <td v-if="item.TGO_FDelete == 0" colspan="8">
        <v-row>
          <v-col cols="7" class="mt-5 text-left">
            <label>بازه تیراژ محصول:</label>
          </v-col>
          <v-col cols="2">
            <v-text-field v-model.number="item.TGO_FNumberMin" class="centered-input" label="حداقل تعداد"
              :disabled="readonly"></v-text-field>
          </v-col>
          <v-col cols="2">
            <v-text-field v-model.number="item.TGO_FNumberMax" class="centered-input" label="حداکثر تعداد"
              :disabled="readonly"></v-text-field>
          </v-col>
          <v-col cols="1" class="mt-5 text-right">
            <span>0 = غیرفعال</span>
          </v-col>
        </v-row>
        <ProductOptions :salePage="salePage" :product="item" :formDefaults="formDefaults" :readonly="readonly">
        </ProductOptions>
      </td>
    </template>
  </v-data-table>
...
import ProductsTableOptionsChip from "./ProductsTableOptionsChip.vue";
