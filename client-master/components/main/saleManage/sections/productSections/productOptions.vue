<template>
  <v-data-table show-expand class="pl-3 pt-5 pr-10 pb-5" style="background: #E1F5FE !important;" :headers="optionHeaders"
    :items.sync="salePage.options" :item-key="get_keyItem()" :expanded.sync="optionExpanded"
    :sort-by.sync="optionsSortBy">


    <template v-slot:item.children="_option">
      <div class="pa-1 ma-1">
        <template v-if="_option.item.TD_FDelete == 0"
         
          v-for="prod_op in return_productOptions(_option.item).sort((a, b) => a.optionValue_TD_FOrder - b.optionValue_TD_FOrder)">
    ...
    <template v-slot:item.TD_FName="{ item }">
      <div class="pa-1 ma-1">
        ...
        <span v-else class="font-italic text-decoration-line-through">{{ item.TD_FName }} (حذف شده)</span>
      </div>
    </template>
    ...
    <template v-slot:expanded-item="{ item }">
      <td v-if="item.TD_FDelete == 0" colspan="8">
        <GoodsOptions :salePage="salePage" :product="product" :option="item" :goodsDefaults="goodsDefaults"
          :formDefaults="formDefaults" :readonly="readonly">
        </GoodsOptions>
      </td>
    </template>
  </v-data-table>
...
import GoodsOptions from "./GoodsOptions.vue";
...
export default {
  ...
  methods: {
    ...
    return_productOptions(salePage_option) {
      ...
      productOptionValues
        .filter(o => o.TGPV_FDelete == 0 && salePage_option.TD_FID == o.TGPV_FID_Option)
        .forEach(prod_op => {
          const index = ret.findIndex(i => i.TGPV_FID_Value == prod_op.TGPV_FID_Value);
          if (index == -1) {
            const optionValue = this.salePage.optionsValues.find(ov => ov.TD_FID == prod_op.TGPV_FID_Value)
  ...
  async mounted() {
    ...
    if (result) {
      this.goodsDefaults = result.data.table.filter(g => g.TGO_FArchive == 1);
    }
  },
  mixins: [goodsMixin, saleManageMixin, saleDataMixin],
  components: { GoodsOptions }
};
</script>
