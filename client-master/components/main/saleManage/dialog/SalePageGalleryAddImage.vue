            ...
            <v-card class="goods_dialog">
                ...
                <v-card-text>
                    ...
                    <v-row>
                        <v-col cols="1"></v-col>
                        <v-col cols="10">
                            <!-- <ui-image-uploader :readonly="readonly" :value="value" accept="jpg, jpeg, png"
                                @input="setUrl" :state="state" /> -->
                                <library-dialog @export="value => setImage(value)" :isAdmin="true" :selected="selected"/>
                        </v-col>
                        <v-col cols="1"></v-col>
                    </v-row>
                    <!-- <v-row>
                    ...
                    <v-row>
                        <v-col cols="6" class="text-right">
                            ...
                            <v-btn v-else-if="showAdd" @click="$emit('submit', img)" dark color="teal">
                                افزودن
                            </v-btn>
                        </v-col>
...
export default {
    ...
    methods: {
        setImage(img){
            this.img.path = img.TPIC_FAddress
            this.img.thumbnail_path = img.thumbnail
            this.img.TPIC_FName = img.TPIC_FName
            this.img.TPIC_FID = img.TPIC_FID
