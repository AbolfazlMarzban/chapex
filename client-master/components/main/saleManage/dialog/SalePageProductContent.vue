        ...
        <v-card-title>
          <v-row>
            <v-col cols="12" class="title">
              ...
              <span v-if="status == 'edit'">ویرایش محصول «{{ product.TGO_FName }}»</span>
        ...
        <v-card-text v-if="product.TGO_FID">
          <v-container>
            <v-expansion-panels v-model="accordianValues" multiple>
              <ProductContentBaseInfo :salePage="salePage" :product="product" :readonly="readonly" :defaults="defaults"
                @errorUpdate="e => (hasError = e)" />

              <ProductContentGallery :salePage="salePage" :product="product" :readonly="readonly" />
            </v-expansion-panels>
...
import ProductContentGallery from "./ProductContentSections/ProductContentGallery.vue";
import ProductContentBaseInfo from "./ProductContentSections/ProductContentBaseInfo.vue";
...
export default {
  ...
  components: { ProductContentGallery, ProductContentBaseInfo }
};
</script>
<style lang="scss">
