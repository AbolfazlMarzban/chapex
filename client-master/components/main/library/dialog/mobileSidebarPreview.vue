<template>
    ...
    <v-row class="pa-5">
      <v-col cols="12" class="d-flex flex-column justify-center px-4">
        <img
          v-if="image && image.TPIC_FAddress.includes('public')"
  ...
  export default {
    props: ["show", "selected","image", "folders", "isAdmin"],
    data() {
      return {
        editName: true,
      editAlt: true,
    ...
    methods: {
      ...
      try{
        ...
        if(result){
          // console.log('getUsedPlaces', result)
          if(result.length){
    ...
    async fileDownload(image) {
      ...
      if (typeof window.navigator.msSaveBlob !== "undefined") {
        ...
        window.navigator.msSaveBlob(blob, image.TPIC_FShowName);
      } else {
        // For other browsers
        ...
        link.href = url;
        link.download = image.TPIC_FShowName; // Set the desired file name
