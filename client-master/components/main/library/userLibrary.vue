          ...
          >
            <v-col cols="12" xl="6" lg="6" md="6" v-if="route">
              <p class="library-breadcrumb mb-0">
                شما اینجا هستید:
                <span
                  @click="$router.push('/admin/userlibrary')"
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  v-if="!libformat"
                >
                  <label>{{ folder.TPIC_FUserOwnerName }}</label>
                </div>
              </div>
            </v-col>
            <v-col
  ...
  export default {
    ...
    methods: {
      ...
      getUsedCapacity() {
        let folder;
        if (this.FID) {
          folder = this.allFolders.find(item => item.TPIC_FUserOwner == this.FID);
        ...
        if (folder) {
          const max = folder.capacity * 1000000;
          let sum = 0;
          if (this.images) {
      ...
      search() {
        ...
        if (this.nameSearch.length > 0) {
          sFolders = this.folders.filter(x =>
            x.TPIC_FUserOwnerName.includes(this.nameSearch)
          );
      ...
      goToFolderBread(folder) {
        this.FID = folder.TPIC_FUserOwner;
        this.getGalleryById(this.FID);
      ...
      async setFile(value) {
        try {
          ...
          let result;
          if (this.FID) {
            result = await this.$authAxios.$post(
              `/gallery/upload/${this.FID}`,
              files
            );
          } else {
            result = await this.$authAxios.$post(`/gallery/upload/null`, files);
          }
          let saveResult;
          if (result) {
            for (var i = 0; i < value.files.length; i++) {
            ...
            saveResult = await this.$authAxios.$post(`/gallery/usersGallerysave?ownerID=${this.FID}`, result);
          }
          if (saveResult) {
            this.loading = false;
      ...
      getID(name, arr, index) {
        if (arr.length > 1) {
          ...
          const folder = this.allFolders.find(item => item.TPIC_FUserOwnerName == name);
          this.FID = folder.TPIC_FUserOwner;
        }
      },
      async getFolders() {
        try {   
            this.allFolders = await this.$authAxios.$get("/gallery/getUsersFolders");
        } catch (error) {
          console.log(error);
        }
      },
      ...
      async getGalleryById(id) {
        try {
            const result = await this.$authAxios.$get(`/gallery/getUsersGalleryByID/${id}`);
            if (result) {
      ...
      async insertFolder(name, capacity, host) {
        try {
          ...
          const res = await this.$authAxios.$post("/gallery/insertFolder", data);
          if (res) {
      ...
      goToFolder(ev, folder) {
        ...
        if (this.isDialog) {
          ...
          this.clickedFolders.push(folder);
            ...
            this.$router.push($nuxt.$route.path + "/" + folder.TPIC_FUserOwnerName);
        }
      },
      async getGallery() {
        try {
            const result = await this.$authAxios.$get("/gallery/users");
            if (result) {
