...
class ErrorController {
    ...
    validationErrorHandler(request, response, next) {
        ...
        if (!validationErrors.isEmpty()) {
            ...
            if (request.file) {
                FileHelper.removeImage(request.file.path)
            }
            if (request.files) {
                for (const fileName in request.files) {
                    FileHelper.removeImage(request.files[fileName][0].path)
                }
            }
            response.status(422).json({
                mesages: this.errorMessages,
                error: true
            })
            return true
        }
    ...
    userNotFoundError(response) {
        response.status(404).json({
            mesages: [this.messages.loginWrongUsernameOrPassword],
            error: true
        })
    }
    ...
    notAuthenticatedError(response) {
        response.status(401).json({
            mesages: [this.messages.notAuthenticated],
            error: true
        })
    }
    ...
    isNotUserError(response) {
        response.status(401).json({
            mesages: [this.messages.isNotUser],
            error: true
        })
    }
    ...
    databaseNotConnected(response) {
        response.status(500).json({
            mesages: [this.messages.databaseNotConnected],
            error: true
        })
    }
    ...
    databaseErrorHandler(response) {
        response.status(500).json({
            mesages: [this.messages.databaseIntenalError],
            error: true
        })
    }

    // responses from database
    responseHandler(result, response, otherData?: any) {
        if (!result) {
